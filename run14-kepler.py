        ...
        for i in range(3900):
            ...
            for i_main, row in train_df.iterrows():
                ...
                for candidate_no, long_answer_candidate in enumerate(row['long_answer_candidates']):
                    ...
                    #print("correct long answer ", row['annotations'][0]['long_answer'])
                    #print("correct short answer ", row['annotations'][0]['short_answers'][0])
		        ...
		        #not 100% sure about the below indices etc. might be wrong
                        if short_end_idx < FLAGS.cont_len:
                            ...
                            for i in range(short_start_idx,FLAGS.cont_len-1):
                                target_present[i] = 1
                
                        #MOVED THE BELOW ONE TAB TO THE RIGHT TO TRAIN ONLY ON THE CORRECT ANSWERS
...
def file_based_convert_examples_to_features(
  ...
  for  example in examples:
    if num_of_examples % 1000 == 0:
      tf.logging.info("Writing example %d " % (num_of_examples))
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    num_of_examples = file_based_convert_examples_to_features(
            processor.get_train_examples(FLAGS.data_dir), label_list, FLAGS.max_seq_length, tokenizer, train_file)
    
    num_train_steps = int(num_of_examples / FLAGS.train_batch_size * FLAGS.num_train_epochs)
  ...
  if FLAGS.do_train:
    ...
    tf.logging.info("  Num examples = %d", num_of_examples)
