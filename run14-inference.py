...
def file_based_convert_examples_to_features(
  ...
  for example in examples:
    ...
    print('feature example id ', features["example_id"])
...
def run_pred():
  ...
  if FLAGS.do_eval:
    ...
    for result in estimator.predict(predict_input_fn, yield_single_examples=True):
        ...
        print("example id type ", type(int(example_id)))
        ...
        if example_id == prev_id or prev_id == 100001:
    ...
    train_df = pd.read_json(FLAGS.data_dir, orient = 'records', lines = True)
    for i_main, row in train_df.iterrows():
        answer_list_of_id = []
        document_text = row['document_text'].split()
        print(i_main)
        ...
        for results in all_results:
            if int(results[0][0]) == int(row['example_id']):
                for candidate_no, long_answer_candidate in enumerate(row['long_answer_candidates']):
                    print("cand list len", len(long_answer_candidate))
                    print("pred list len", len(results))
                    print("doc ex id", row['example_id'])
                    print("pred ex id", results[candidate_no])
                    ...
                    print("score: ", results[i_main][candidate_no][-1])
